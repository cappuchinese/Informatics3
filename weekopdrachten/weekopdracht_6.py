...
class Nucliotides:
    ...
    def number_listing(self):
        ...
        “4: Thymine”,
        """
        for num, nuc in enumerate(self.__nucs):
            return f"{num+1}: {nuc}"
    ...
    def filter_pyrimidines(self):
        ...
        “[‘Cytosine’, ‘Thymine’]”
        """
        return list(filter(lambda x: x in self.__nucs, self.__pyrimidines))
...
class Somequiz:
    ...
    def q2(self):
        ...
        """
        given = [(2, 1, 3), (1, 5, 3, 1), (7, 3, 5), (6, 8), (1, 4, 2)]
        maxs = list(map(lambda x: max(x), given))
        avs = list(map(lambda x: sum(x)/len(x), given))
        return maxs, avs
...
class Seqreq:
    ...
    def sort_assession(self):
        ...
        """
        return sorted(self.seqrec, key= lambda x: x["accno"][3:])
